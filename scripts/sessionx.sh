...
handle_output() {
	...
	if ! tmux has-session -t="$target" 2>/dev/null; then
		if is_tmuxinator_enabled && is_tmuxinator_template "$target"; then
			tmuxinator_args_str="$(tmux show-option -gqv @sessionx-tmuxinator-args)"
			tmuxinator_args=()
			if [[ -n "$tmuxinator_args_str" ]]; then
				while IFS= read -r arg; do
					# Most of the args to start are either irrelevant or conflict with switch-client
					# Only accept the args which would be relevant
					case "$arg" in
					'--suppress-tmux-version-warning' | '--no-pre-window')
						tmuxinator_args+=("$arg")
						;;
					esac
				done < <(xargs -n1 <<<"$tmuxinator_args_str")
			fi
			tmuxinator start "$target" "${tmuxinator_args[@]}"
